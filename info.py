...
CHAT_GROUP = int(environ.get("CHAT_GROUP", "-1002082710490"))

# Admins, Channels & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '330029937').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001958911015').split()]
...
auth_channel = environ.get('AUTH_CHANNEL', '-1001958911015') #Channel / Group Id for force sub ( make sure bot is admin )
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002055042397') # support group id ( make sure bot is admin )
...
S_GROUP = environ.get('S_GROUP', "https://t.me/movieverse_discussion_2")
...
MSG_ALRT = environ.get('MSG_ALRT', 'ᴍᴀɪɴᴛᴀɪɴᴇᴅ ʙʏ : MovieVerse')
